...
function routeName(p){
  if(p.includes("/components/"))return "(component) "+p.split("/").pop();
  const rel=p.replace(ROOT+"/app/","").replace("/page.tsx","").replace("page.tsx","");
  return "/"+rel;
} 

function clean(t){
  return t.replace(/&mdash;/g," ").replace(/&amp;/g,"&").replace(/&nbsp;/g," ").replace(/&[a-z]+;/g,"").replace(/\s+/g," ").trim();
} 
...
function extract(src){   
  ...
  // FIX: Convert React curly-brace string literals (like {" "}) into raw text 
...
for(const f of files){
  const lines=extract(readFileSync(f,"utf8"));
